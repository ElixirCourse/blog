...
Съществували са *keyword list*-овете, които са просто списъци от наредени двойки елементи във вида `{атом, стойност}`.
Имало е и *records*, които са подобни на `struct`-овете в *C*. Всъщност тези *record* структури са просто именувани кортежи.
Можете да научите повече за тях [тук](http://erlang.org/doc/reference_manual/records.html).
...
Ние ще си говори за *Map* типа, който се създава с `%{}`.
Не използвайте `HashDict` и `Dict`.
Избягвайте да използвате *keyword lists* вместо *Map*.
...
Когато можем да използваме атоми е най-добре да използваме само атоми.
...
Не е нужно лявата и дясната страна на израза, които се съпоставят да съвпадат изцяло.
