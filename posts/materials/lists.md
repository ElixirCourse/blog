...
В горния пример, използваме специален оператор (`|`) за да опишем каква е стойността на елемента и какъв е остатъкът от списъка.
Горният ред казва: главата на списъка съдържа `1` и сочи към остатъка от списъка: `[2 | [3 | [4 | [5 | []]]]]`,
който е списък с глава, която съдържа `2` и сочи към остатъка на списъка: `[3 | [4 | [5 | []]]]` и т.н.
Последният елемент е `[5 | []]`, който съдържа `5` и сочи към остатък, който е празен списък. Това маркира края на данните.

Друг начин за да се опише горната структура е: `[head | tail]`, където `head` е стойността на главата на списъка, а `tail` е остатъка.
Това е и синтаксиса, ако искаме да *pattern match*-ваме списъци:
...
* Ако списъкът не е празен, то можем да го разделим на глава и остатък. Дължината му ще бъде **1 + <дължината на остатъка>**.
...
Обикновено имаме версия на операцията върху празен списък, която се явява дъно на рекурсията или край на операцията и връщане на резултат
и една или повече версии на операцията които прилагат операцията върху главата на списъка и рекурсивно извикват операцията над остатъка.
Това е много често използван шаблон при обработването на списъци.

## Рекурсивно изграждане на списък

Аналогично можем и да изграждаме списъци чрез рекурсия, като ги дефинираме като "глава" и "остатък":
...
* Списъкът от квадрати на не-празен списък е списъкът от квадрата на първия елемент с остатък списъкът от квадрати на остатъка на оригиналния списък.

Така рекурсивно можем да изграждаме списъци от други списъци.

Ако имаме един списък и направим два нови списъка, използвайки първия за остатък, то ние ще пре-използваме паметта на основния списък:
...
Това се дължи на неизменимостта на данните в *Elixir*.
...
Също така повечето такива имплементации са *BIF*-ове (*Built-In Functions*) и имат добра производителност.
...
Един списък от числа ще бъде винаги представян като низ в единични кавички, ако тези числа са *printaple ASCII condepoint*-и:
...
Запомнете, че когато използвате *Erlang*-ски функции, които би трябвало да изискват низ, те ще изискват *charlist* и може да се наложи
трансформация от стринг към *charlist*. Има функция, която може да прави това : `String.to_charlist/1`.
...
Опитахме се да покрием максимално подробно темата за списъците, работат с тях и тяхната имплементация.
В следващата публикация ще представим типа *tuple* или кортеж и типа *keyword list*, който представлява списък от кортежи с по два елемента всеки (първият елемент на всеки такъв кортеж трябва да е атом).
