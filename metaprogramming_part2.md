...
Последно се запознахме с AST-то на Elixir, видяхме [как прилича на LISP](https://www.youtube.com/watch?v=IZvpKhA6t8A) и как да боравим с него. Научихме се да използваме `quote` и `unquote`. Обърнахме операциите плюс и умножение с минус и деление. Накрая си написахме наша версия на `while` цикъл.  

# Какво ще правим във втората част за метапрограмиране?

Днеска ще се запознаем с това какво са модулни атрибути, ще разгледаме различни compile-time hooks, научим да използваме `bind_quoted` и `__using__`. Ще поговорим за това как да пишем `чисти` макрота и ще разгледаме `var!`. В процеса на работа, ще научим повече за това как е написан еликсир и колко лесно можем сами да си разширяваме езика, като сами си напишем `unless` макрото и разгледаме други примери в ядрото на Еликсир. Накрая ще си напишем собствена версия на фреумърк за тестове, в процеса ще се сблъскаме с проблеми около писането на макрото.


# `Unless` макрото.

Нека започнем с нещо лесно, а именно `unless` макрото. На кратко `unless` изпълнява даден блок, когато дадено условие е лъжа(обратното на един `if`). Да видим набързо AST-то на един `if`.
...
```
 ...
 От миналия път знаем, че `...макросите са функции, които приемат като аргумент AST и връщат AST...`. Тогава като гледаме AST-то на `if`-а, трябва просто да обърнем стойност на условието, примерно използвайки `!` оператора и да върнем почти същото AST. Нека видим това как ще стане. Започваме като си дефинираме макро като използваме `quote`, `unquote` и макрото `defmacro` (So meta right?).
...
{:if, [context: Conditions, import: Kernel], [false, [do: "Hello"]]}
 ...
 Какво виждаме, като веднъж разгънем макрото(като използвахме [`Macro.expand_once`](https://hexdocs.pm/elixir/Macro.html#expand_once/2)), то се е генерирало до нормален `if`. Всъщност може да считаме, че когато Еликсир види макро, почва да го разгъва рекурсивно, докато повече не може. Може да разгледате [`Macro`](https://hexdocs.pm/elixir/1.0.5/Macro.html) и [`Code`](https://hexdocs.pm/elixir/1.0.5/Code.html) модулите за повече информация.
...
Не ни вярвате, че if–овете са истина за всичко, освен `nil/false`? Вече няма нужда да ни се доверявате, може да разгледате сами как са реализирани всички яки работи. И ето малко примери:
...
Често в макротата използваме `unquote`, за да оценим някаква променлива в подадения ни контекст:
...
Тука забелязахме нещо важно, `name`, което беше дефинирано вътре в макрото го няма отвън. Това е част от чистотата на макротата, за която ще си говорим малко по-късно.
...
# Чистота на макротата.

Като пишем макрота в Еликсир, не генерираме само код, ние го инжектираме в контекста подаден ни от извикващия човек. Контекста държи локалния `binding`, импортите и алиасите на човека. Един вид контекстът е нашия свят, който виждаме вътре от макрото, за това е то толкова важен.

Нека видим как Еликсир ни предпазва от това да "замърсяваме" средата в която сме:

```
iex(1)> ast = quote do
...(1)>   if a == 4 do
...(1)>     "Look out, we've got a four here."
...(1)>   else
...(1)>     "Mehhh"
...(1)>   end
...(1)> end
iex(2)> Code.eval_quoted ast, a: 4
...
Като използваме `var!` макрото, можем явно да предефинираме локалния binding в контекста - подаден ни в макрото. По този начин казваме на Еликсир: "Знам какво правя, не се притеснявай, тука това ще го използвам." Нека накараме предишния пример да работи:
...
{:if, [context: Elixir, import: Kernel],
 [{:==, [context: Elixir, import: Kernel],
   [{:var!, [context: Elixir, import: Kernel], [{:a, [], Elixir}]}, 4]},
  [do: "Look out, we've got a four here.", else: "Mehhh"]]}
iex(2)> Code.eval_quoted ast, a: 4
{"Look out, we've got a four here.", [a: 4]}
...
# Наш собствен фреймуърк за тестове.

Добре, вече знаем как да използваме `quote/unquote/bind_quoted`, `var!`. Ще се опитаме да си напишем собствена библиотека за тестове с малък приятен DSL взаимстван от `exunit`. За целта ще се опитаме да предоставим следните неща:
...
Добре, нека започнем първо с `__using__`  - макро, което ни дава да дефинираме callback, когат някой ни използва модула. На кратко, ако имаме:
...
Това ни позволява лесно да вкараме методите и макротата за тестове, които ще напишем в модула на потребителя.

Как ще го постигнем това? Като предефинираме макрото `__using__`. Просто искаме да `import`-нем нашия модул, можем да използваме [`__MODULE__`](https://hexdocs.pm/elixir/Kernel.SpecialForms.html#__MODULE__/0).
...
Така ще можем да импортираме всички функции от модулите ни, когато човек иска да се възползва от тях. Само ще трябва да напише `use Assertion`.
...
Добре, вече имаме assertion-и, сега трябва да дефинираме начин да пишем тестовете. Накратко искаме да дефинираме някакво макро `spec`, което ще приема низ(описанието на теста) и блок, който да изпълним след това. Когато потребителят използва `spec` ще дефинираме функция, с името на дескрипшъна и ще си записваме някъде всички `spec`-ове, които хората са си дефинирали. Когато човек иска да пусне тестовете, ще минаваме през всички наши записи и ще ги пускаме. За целта да ги записваме ще използваме модулни атрибути.
...
Как ще пазим всички написани тестове? Ще инициализираме модулен атрибут, който ще е празен лист. В този лист ще записваме всеки използван тест. За да не презаписваме предишния тест, ще добавяме в началото всяка двойка от име на тест и описание.
...
defmodule Specs do
  defmacro __using__(_options) do
    quote do
      ...
      # Инициализираме празен лист като модулен атрибут.
      @specs []
...
Нека реализираме последния нужен модул, който автоматично ще дефинира функцията `run` в ExampleTests. За жалост не може просто да си отворим модула `ExampleTests` и да му добавим функцията `run`. За целта ще използваме [`__before_compile__`](https://hexdocs.pm/elixir/1.0.5/Module.html).
